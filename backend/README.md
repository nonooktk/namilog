...
- **危機検知の窓口文言**: `services/support_messages.py` の定数に集約。**確定済み（2026-07-12・
  デザイン仕様 6.5・厚労省公式で一次確認）**: よりそいホットライン 0120-279-338（24時間・無料）／
  こころの健康相談統一ダイヤル 0570-064-556（受付時間は地域により異なる）。番号・受付時間に変更が
  あればこの1ファイルのみ差し替える。
